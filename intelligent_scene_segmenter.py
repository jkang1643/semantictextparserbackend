"""
Intelligent scene-based text segmentation module.

This module implements intelligent scene segmentation that groups text into coherent scenes
based on subject matter, transition words, and semantic context changes. It focuses on
creating meaningful scene boundaries rather than granular sentence-level segmentation.
"""
...
class IntelligentSceneSegmenter:
    """
    Intelligent scene-based text segmentation.
    
    Segments text into coherent scenes based on:
    - Subject/entity shifts (character, place, object focus changes)
    - Transition cue words (temporal, causal, narrative markers)
    - Semantic divergence from scene context
    """
    ...
    def __init__(self, 
        ...
        """
        Initialize the intelligent scene segmenter.
        ...
        # Only the strongest transition cues that indicate scene changes
        self.transition_cues = [
            'suddenly', 'abruptly', 'instantly', 'immediately',
            'meanwhile', 'elsewhere', 'however', 'but', 'yet'
        ]
        
        # Compile regex pattern for transition cue detection
        self.transition_pattern = re.compile(
            r'\b(' + '|'.join(self.transition_cues) + r')\b',
            re.IGNORECASE
        )
    
    def segment_scenes(self, text: str, similarity_threshold: Optional[float] = None) -> List[str]:
        """
        Segment text into coherent scenes based on subject matter and narrative flow.
        ...
        for i in range(1, len(sentences)):
            ...
            should_break_scene = self._should_break_scene(
                sentence, embeddings[i], scene_context, similarity_threshold
            )
    ...
    def _should_break_scene(self, 
        ...
        """
        # Check for transition cue words (hard boundary)
        if self._has_transition_cues(sentence):
            return True
        
        # Check for subject/entity shift
        sentence_subjects = self._extract_subjects(sentence)
        sentence_entities = self._extract_entities(sentence)
        
        # Check if main subject changes
        if self._has_subject_shift(scene_context['subjects'], sentence_subjects):
            return True
        
        # Check if dominant entity changes
        if self._has_entity_shift(scene_context['entities'], sentence_entities):
            return True
    ...
    def _has_transition_cues(self, sentence: str) -> bool:
        """Check if sentence starts with transition cue words."""
        # Check if sentence starts with a transition cue
        sentence_lower = sentence.lower().strip()
        for cue in self.transition_cues:
            if sentence_lower.startswith(cue):
                return True
        return False
    
    def _has_subject_shift(self, current_subjects: Set[str], new_subjects: Set[str]) -> bool:
        """Check if the main subject has changed."""
        if not current_subjects or not new_subjects:
            return False
        
        # If new sentence has subjects but none overlap with current scene
        return not current_subjects.intersection(new_subjects)
    
    def _has_entity_shift(self, current_entities: Set[str], new_entities: Set[str]) -> bool:
        """Check if dominant entities have changed significantly."""
        if not current_entities or not new_entities:
            return False
        
        # Calculate overlap ratio
        overlap = len(current_entities.intersection(new_entities))
        total_unique = len(current_entities.union(new_entities))
        
        # If less than 30% overlap, consider it a significant shift
...
def segment_scenes(text: str, similarity_threshold: float = 0.4) -> List[str]:
    """
    ...
    """
    segmenter = IntelligentSceneSegmenter(similarity_threshold=similarity_threshold)
    return segmenter.segment_scenes(text, similarity_threshold)


def segment_scenes_with_titles(text: str, similarity_threshold: float = 0.65) -> Tuple[List[str], List[str]]:
    """
